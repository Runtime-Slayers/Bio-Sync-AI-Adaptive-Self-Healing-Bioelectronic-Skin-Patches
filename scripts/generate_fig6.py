import matplotlib.pyplot as plt
import numpy as np
import csv

# Data
systems_lat = ['BioSync-AI\n(On-Edge)', 'Cloud-Based\nWearable', 'Traditional\nMed Pump', 'Manual\nHospital']
latency = [8.4, 320, 480, 3600]

with open('../data/latency_comparison.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['System', 'Latency (ms)'])
    for s, l in zip(systems_lat, latency):
        writer.writerow([s.replace('\n', ' '), l])

systems_pow = ['BioSync-AI (15W peak)', 'NVIDIA Jetson Xavier (30W)', 'Cloud-Based System (85W)']
power = [15, 30, 85]

with open('../data/power_comparison.csv', 'w', newline='') as f:
    writer = csv.writer(f)
    writer.writerow(['System', 'Power (W)'])
    for s, p in zip(systems_pow, power):
        writer.writerow([s, p])

# Plotting
fig, axs = plt.subplots(1, 2, figsize=(14, 6))

# (a) Latency
y_pos = np.arange(len(systems_lat))
axs[0].barh(y_pos, latency, align='center', color=['#2196F3', '#FF9800', '#9C27B0', '#F44336'])
axs[0].set_yticks(y_pos)
axs[0].set_yticklabels(systems_lat)
axs[0].invert_yaxis()  # labels read top-to-bottom
axs[0].set_xlabel('Response Latency (ms)')
axs[0].set_title('(a) Therapeutic Response Latency', fontweight='bold')
axs[0].set_xscale('log')

for i, v in enumerate(latency):
    axs[0].text(v + (v * 0.1), i, f'{v}ms', va='center')

# (b) Power
colors = ['#2196F3', '#9C27B0', '#FF9800']
explode = (0.1, 0, 0)
axs[1].pie(power, explode=explode, labels=systems_pow, colors=colors,
        autopct='%1.1f%%', shadow=False, startangle=140)
axs[1].axis('equal')
axs[1].set_title('(b) Relative Power Consumption (W)', fontweight='bold')

plt.tight_layout()
plt.savefig('../figures/fig6_latency_power.png', dpi=300)
